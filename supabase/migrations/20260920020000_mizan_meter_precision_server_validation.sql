...
-- Final override of record_water_reading: preserve tenant/owner authority,
-- idempotency, pending-reading race protection, and add server-side precision checks.
-- Client-side OCR/input validation is never trusted.
...
BEGIN
  ...
  IF v_profile_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    RAISE EXCEPTION 'Meter precision profile is missing';
  END IF;
...
REVOKE ALL ON FUNCTION public.record_water_reading(
  UUID,NUMERIC,TEXT,TEXT,TEXT,NUMERIC,TEXT,TEXT,NUMERIC,NUMERIC,NUMERIC
) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.record_water_reading(
  UUID,NUMERIC,TEXT,TEXT,TEXT,NUMERIC,TEXT,TEXT,NUMERIC,NUMERIC,NUMERIC
) TO authenticated;
