...
BEGIN
  ...
  IF p_status NOT IN ('active', 'suspended') THEN RAISE EXCEPTION 'invalid subscription status'; END IF;
  ...
  VALUES (v_row.id,auth.uid(),'subscription_status_changed','tenant',v_row.id,
...
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'authentication required'; END IF;
  IF NOT public.is_super_admin() THEN RAISE EXCEPTION 'super-admin authorization required'; END IF;
  IF btrim(coalesce(_tenant_id, '')) = '' THEN RAISE EXCEPTION 'tenant identifier is required'; END IF;
  IF btrim(coalesce(_license_key, '')) = '' THEN RAISE EXCEPTION 'license key is required'; END IF;
  IF _max_seats IS NULL OR _max_seats < 1 OR _max_seats > 10000 THEN RAISE EXCEPTION 'invalid seat count'; END IF;
  IF _days IS NULL OR _days < 1 OR _days > 3650 THEN RAISE EXCEPTION 'invalid license duration'; END IF;
  ...
  VALUES (v_uuid,_tenant_id,'active'::public.subscription_status,now()+make_interval(days=>_days))
  ...
  INSERT INTO public.audit_logs (tenant_id,user_id,action,entity,entity_id,meta)
  VALUES (v_row.id,auth.uid(),'tenant_activated','tenant',v_row.id,
          jsonb_build_object('max_seats',_max_seats,'days',_days,'license_key_supplied',true));

  RETURN v_row;
END;
$$;

REVOKE ALL ON FUNCTION public.set_tenant_subscription_status(uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.set_tenant_subscription_status(uuid, text) TO authenticated;
REVOKE ALL ON FUNCTION public.activate_tenant(text, text, integer, integer) FROM PUBLIC, anon;
