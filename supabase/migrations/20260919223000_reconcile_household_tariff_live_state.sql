-- Reconcile the household/tariff rollout after an interrupted remote execution.
-- This migration is intentionally idempotent and rolls forward only.
-- The canonical schema remains 20260919210000_household_tariff_sustainability.sql.
-- It exists so a live database that received only the first part of that migration
-- can be brought to the same verified state without destructive resets.
...
FOR EACH ROW EXECUTE FUNCTION public.apply_configured_water_tariff();
...
AS
WITH production AS (
  SELECT tenant_id, date_trunc('month', recorded_at)::date AS month_start,
...
billed AS (
  SELECT b.tenant_id, date_trunc('month', b.issued_at)::date AS month_start, SUM(b.total) AS billed_amount
  FROM public.water_bills b
  JOIN public.water_readings r ON r.id = b.reading_id AND r.tenant_id = b.tenant_id
    AND r.verification_status = 'approved' AND r.status = 'approved'
  WHERE b.tenant_id = public.current_tenant_id()
  GROUP BY b.tenant_id, date_trunc('month', b.issued_at)
),
collected AS (
  SELECT p.tenant_id, date_trunc('month', p.created_at)::date AS month_start, SUM(p.amount) AS collected_amount
  FROM public.payments p
  JOIN public.water_bills b ON b.id = p.bill_id AND b.tenant_id = p.tenant_id
  JOIN public.water_readings r ON r.id = b.reading_id AND r.tenant_id = b.tenant_id
    AND r.verification_status = 'approved' AND r.status = 'approved'
  WHERE p.tenant_id = public.current_tenant_id() AND p.status = 'approved' AND p.amount >= 0
  GROUP BY p.tenant_id, date_trunc('month', p.created_at)
),
months AS (
  SELECT tenant_id, month_start FROM production
  UNION SELECT tenant_id, month_start FROM readings
  UNION SELECT tenant_id, month_start FROM billed
  UNION SELECT tenant_id, month_start FROM collected
)
SELECT m.tenant_id, m.month_start,
       COALESCE(pr.system_input_m3, 0)::numeric AS system_input_m3,
       COALESCE(rd.consumption_m3, 0)::numeric AS consumption_m3,
       CASE WHEN COALESCE(pr.system_input_m3, 0) > 0
         THEN (pr.system_input_m3 - COALESCE(rd.consumption_m3, 0))::numeric ELSE NULL END AS metered_balance_gap_m3,
       CASE WHEN COALESCE(pr.system_input_m3, 0) > 0
         THEN ((pr.system_input_m3 - COALESCE(rd.consumption_m3, 0)) / pr.system_input_m3 * 100)::numeric ELSE NULL END AS metered_balance_gap_pct,
       CASE WHEN COALESCE(pr.system_input_m3, 0) > 0
         THEN (COALESCE(rd.consumption_m3, 0) / pr.system_input_m3 * 100)::numeric ELSE NULL END AS water_efficiency_pct,
       COALESCE(bl.billed_amount, 0)::numeric AS billed_amount,
       COALESCE(co.collected_amount, 0)::numeric AS collected_amount,
       CASE WHEN COALESCE(bl.billed_amount, 0) > 0
         THEN (COALESCE(co.collected_amount, 0) / bl.billed_amount * 100)::numeric ELSE NULL END AS collection_rate_pct,
       COALESCE(rd.approved_readings, 0)::bigint AS approved_readings,
       COALESCE(rd.total_readings, 0)::bigint AS total_readings,
       CASE WHEN COALESCE(rd.total_readings, 0) > 0
         THEN (rd.approved_readings::numeric / rd.total_readings * 100) ELSE NULL END AS approval_rate_pct
FROM months m
