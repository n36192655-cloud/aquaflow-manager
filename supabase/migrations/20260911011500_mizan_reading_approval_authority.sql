...
-- Capture is a pending transaction. Approval is a separate server-authorized transition.
-- The invoice is created only when the reading is approved.
...
-- Existing legacy rows are not silently promoted. They remain pending until explicitly reviewed.
...
RETURNS TABLE(reading_id UUID, previous NUMERIC, current_reading NUMERIC, consumption NUMERIC, status TEXT, verification_status TEXT)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
...
BEGIN
  ...
  IF p_client_id IS NOT NULL THEN
    ...
    IF v_reading IS NOT NULL THEN
      SELECT wr.id, wr.previous, wr.current_reading, wr.consumption, wr.status, wr.verification_status
      INTO reading_id, previous, current_reading, consumption, status, verification_status
      FROM public.water_readings wr WHERE wr.id = v_reading;
      RETURN NEXT; RETURN;
    END IF;
  END IF;
  ...
  VALUES(
    ...
    v_consumption, p_photo_url, p_lat, p_lng, 'ok', 'pending', v_uid, p_capture_source,
    p_ocr_serial, p_ocr_confidence, p_ocr_raw_text,
...
BEGIN
  ...
  SELECT wr.customer_id, wr.consumption, wr.verification_status
  INTO v_customer, v_consumption, v_status
  FROM public.water_readings wr
  ...
  -- Recalculate from server-side tariff and current outstanding ledger.
  ...
  WHERE id=p_reading_id AND tenant_id=v_tenant;
  ...
  SELECT v_tenant, v_customer, wr.id, v_subtotal, v_arrears, v_subtotal + v_arrears, 'unpaid', v_cycle, v_project, wr.client_id
  FROM public.water_readings wr
...
BEGIN
  ...
  UPDATE public.water_readings
  SET verification_status='rejected', status='rejected', rejection_reason=trim(p_reason), verified_by=v_uid, verified_at=now()
...
-- Browser writes must not bypass the authoritative workflow.
