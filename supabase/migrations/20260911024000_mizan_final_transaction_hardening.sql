-- MIZAN final transaction hardening.
-- This is the final authoritative layer for field readings and payments.
-- No service-role path is introduced; all browser mutations remain RPC-only.
...
-- SECURITY DEFINER billing-cycle creation must never accept an arbitrary tenant id
...
BEGIN
  ...
  END IF;
  IF p_tenant IS NULL OR (p_tenant <> v_current_tenant AND NOT public.is_super_admin()) THEN
    RAISE EXCEPTION 'Tenant scope mismatch';
  END IF;
  IF NOT (public.has_tenant_role(p_tenant,'manager') OR public.is_super_admin()) THEN
...
-- Server-authoritative reading capture. The advisory lock serializes captures for
-- one meter so concurrent field devices cannot derive the same previous reading.
...
BEGIN
  ...
  END IF;
  IF p_ocr_confidence IS NOT NULL AND (p_ocr_confidence < 0 OR p_ocr_confidence > 1 OR p_ocr_confidence = 'NaN'::numeric) THEN
  ...
  SELECT m.customer_id, m.serial_number
  INTO v_customer, v_serial
  ...
  IF v_customer IS NULL THEN
    RAISE EXCEPTION 'Meter is not active or is not assigned to this tenant';
  END IF;
  ...
  IF v_client_id IS NOT NULL THEN
    ...
    WHERE wr.tenant_id = v_tenant
      AND wr.client_id = v_client_id;

    IF v_existing.id IS NOT NULL THEN
      IF v_existing.meter_id <> p_meter_id
         OR v_existing.current_reading <> p_current THEN
      ...
      SELECT v_existing.id, wb.id, v_existing.previous, v_existing.current_reading,
             v_existing.consumption, wb.total, wb.arrears,
  ...
  INSERT INTO public.water_readings(
    tenant_id, customer_id, meter_number, meter_id, previous, current_reading,
    consumption, photo_url, lat, lng, flag, status, reader_id, capture_source,
    ocr_serial, ocr_confidence, ocr_raw_text, identity_verified, reading_verified,
    client_id, source_device, verification_status
  ) VALUES (
    v_tenant, v_customer, v_serial, p_meter_id, v_prev, p_current,
    p_current - v_prev, p_photo_url, p_lat, p_lng,
    CASE WHEN p_current = v_prev THEN 'zero' ELSE 'ok' END,
    'pending', v_uid, p_capture_source, p_ocr_serial, p_ocr_confidence,
    p_ocr_raw_text, p_ocr_serial IS NOT NULL, false, v_client_id, NULL, 'pending'
  )
  ...
  IF v_reading IS NULL AND v_client_id IS NOT NULL THEN
    ...
    IF v_existing.id IS NULL THEN
      RAISE EXCEPTION 'Reading idempotency conflict could not be resolved';
    END IF;
...
-- Payment creation is serialized on the bill and has strict idempotency semantics.
...
BEGIN
  ...
  IF v_client_id IS NOT NULL THEN
    ...
    IF v_existing.id IS NOT NULL THEN
      ...
      END IF;
      payment_id := v_existing.id; bill_id := v_existing.bill_id;
      amount := v_existing.amount; status := v_existing.status;
      RETURN NEXT;
      RETURN;
    END IF;
  END IF;
  ...
  INSERT INTO public.payments(
    tenant_id, bill_id, amount, method, status, collector_id, client_id
  ) VALUES (
    v_tenant, p_bill_id, p_amount, p_method, 'pending', v_uid, v_client_id
  )
  ON CONFLICT (tenant_id, client_id) WHERE client_id IS NOT NULL DO NOTHING
  ...
  VALUES(v_tenant,v_uid,'payment_created','payment',payment_id::text,
    jsonb_build_object('bill_id',bill_id,'amount',amount,'method',p_method,'status',status));
...
BEGIN
  ...
  SET status='approved', approved_by=v_uid, approved_at=now(),
      rejected_at=NULL, review_reason=NULL
  ...
  SELECT p.* INTO v_payment FROM public.payments p WHERE p.id=p_payment_id AND p.tenant_id=v_tenant;
  RETURN v_payment;
END;
$$;
...
BEGIN
  ...
  UPDATE public.payments
  SET status='rejected', review_reason=trim(p_reason), rejected_at=now(),
...
-- Recompute only from approved payments and only inside the supplied tenant.
...
BEGIN
  ...
  IF v_current_tenant IS NULL OR (p_tenant_id <> v_current_tenant AND NOT public.is_super_admin()) THEN
    RAISE EXCEPTION 'Tenant scope mismatch';
  END IF;
  IF NOT (public.has_tenant_role(p_tenant_id,'manager') OR public.is_super_admin()) THEN
    RAISE EXCEPTION 'Manager permission required';
  END IF;
