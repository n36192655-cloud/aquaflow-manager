...
function PaymentsPage() {
  ...
  async function load() {
    ...
    const [p, b, c] = await Promise.all([
      supabase
        .from("payments")
        .select("id,bill_id,amount,status,method,collector_id,created_at,review_reason")
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`mizan-payments-${user?.tenantId ?? "none"}`)
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  async function approve(id: string) {
    setBusy(true);
    try {
      await approveWaterPayment(id);
      toast.success("تم اعتماد الدفعة وإعادة احتساب رصيد الفاتورة من سجل الدفعات");
      await load();
    } catch (error) {
      toast.error(`فشل اعتماد الدفعة: ${(error as Error).message}`);
    } finally {
      setBusy(false);
    }
  }
  ...
  async function reject(id: string) {
    ...
    setBusy(true);
    try {
      await rejectWaterPayment(id, reason);
      toast.info("تم رفض الدفعة وتسجيل السبب");
      await load();
    } catch (error) {
      toast.error(`فشل رفض الدفعة: ${(error as Error).message}`);
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            الدفعة لا تصبح تحصيلاً فعلياً إلا بعد اعتمادها من الإدارة.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Stat label="تحصيل معتمد" value={fmtYER(approvedTotal)} />
        <Stat label="معلّق" value={fmtYER(pendingTotal)} />
      ...
      {payments.length === 0 && (
        <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900 flex gap-2">
          <AlertCircle className="w-5 h-5" /> لا توجد دفعات حقيقية مسجلة حالياً.
        </div>
      )}
      <div className="flex gap-2">
        {(["pending", "approved", "rejected"] as const).map((t) => (
          <Button
            ...
            onClick={() => setTab(t)}
          >
            {t === "pending" ? "بانتظار الاعتماد" : t === "approved" ? "معتمدة" : "مرفوضة"}
          </Button>
        ))}
      </div>
      <Card>
                ...
                return (
                  <TableRow key={payment.id}>
                    <TableCell className="text-xs">
                      {new Date(payment.created_at).toLocaleString("ar-YE")}
                    </TableCell>
                    <TableCell>{customer?.name ?? "—"}</TableCell>
                    <TableCell className="font-mono text-xs">{payment.bill_id}</TableCell>
                    <TableCell className="font-semibold">{fmtYER(payment.amount)}</TableCell>
                    <TableCell>
                      <Badge variant="outline">
                        {payment.method === "cash" ? "نقدي" : "تحويل بنكي مسجل"}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <Badge
                      ...
                      {payment.review_reason && (
                        <div className="text-xs text-destructive mt-1">{payment.review_reason}</div>
                      )}
                    </TableCell>
                    {tab === "pending" && isAdmin && (
                      <TableCell>
