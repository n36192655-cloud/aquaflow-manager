...
async function loadMetrics(): Promise<Metrics> {
  ...
  const kpiResult = await supabase.from("monthly_water_service_kpis").select("month_start,system_input_m3,consumption_m3,nrw_m3,nrw_pct,water_efficiency_pct,billed_amount,collected_amount,collection_rate_pct,approved_readings,total_readings,approval_rate_pct").eq("tenant_id", tenantId).order("month_start", { ascending: true });
  let monthlySource = monthlyTrend;
  if (!kpiResult.error && Array.isArray(kpiResult.data)) {
    monthlySource = (kpiResult.data as MonthlyKpi[]).map((x) => ({
      month: x.month_start.slice(0, 7),
      consumption: Number(x.consumption_m3),
      production: Number(x.system_input_m3),
      nrw: x.nrw_pct == null ? null : Number(x.nrw_pct),
      billed: Number(x.billed_amount),
      collected: Number(x.collected_amount),
    }));
  } else {
    alerts.push("مؤشرات KPI الشهرية المحسوبة داخل قاعدة البيانات غير متاحة حالياً؛ عُرضت مؤشرات احتياطية من البيانات المعتمدة.");
  }
  return { tenantName: tenant.data?.name ?? "المشروع الحالي", windowStart: startIso, windowEnd: new Date(end.getTime() - 1).toISOString(), approvedConsumption, productionInput, waterEfficiency: productionInput && productionInput > 0 ? approvedConsumption / productionInput * 100 : null, nrw: productionInput && productionInput > 0 ? (productionInput - approvedConsumption) / productionInput * 100 : null, approvedPayments: approvedPaymentTotal, eligibleBilled, collectionRate: eligibleBilled > 0 ? approvedPaymentTotal / eligibleBilled * 100 : null, approvedReadings: approvedReadings.length, totalReadings: readings.length, operationalEfficiency: readings.length ? approvedReadings.length / readings.length * 100 : null, productionAvailable, activeCustomers: customerRows.length, representedPeople, avgPerCapitaLpd, monthlyTrend: monthlySource, consumptionTrend, productionTrend, nrwTrend, financialTrend, workflow: { pending, approved: approvedReadings.length, rejected }, alerts };
}
