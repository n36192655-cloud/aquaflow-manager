...
function Dashboard() {
  const { user } = useAuth(); const [m, setM] = useState(empty); const [isCentralTenant, setIsCentralTenant] = useState(false); const [loading, setLoading] = useState(true); const [refreshing, setRefreshing] = useState(false); const [error, setError] = useState<string | null>(null); const [live, setLive] = useState(false);
  ...
  useEffect(() => { const tenantId = user?.tenantId; if (!tenantId) return; void (async () => { const { data } = await supabase.from("tenants").select("tenant_type").eq("id", tenantId).maybeSingle(); setIsCentralTenant(data?.tenant_type === "central"); })(); }, [user?.tenantId]);
  useEffect(() => { if (isCentralTenant) { setLoading(false); return; } void refresh(); const c = supabase.channel("mizan-dashboard").on("postgres_changes", { event: "*", schema: "public", table: "water_readings" }, () => void refresh()).on("postgres_changes", { event: "*", schema: "public", table: "water_bills" }, () => void refresh()).on("postgres_changes", { event: "*", schema: "public", table: "payments" }, () => void refresh()).on("postgres_changes", { event: "*", schema: "public", table: "water_production_logs" }, () => void refresh()).subscribe((s) => setLive(s === "SUBSCRIBED")); return () => { void supabase.removeChannel(c); }; }, [refresh]);
