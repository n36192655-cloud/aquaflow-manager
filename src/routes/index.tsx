...
function Dashboard() {
  ...
  useEffect(() => { if (!user?.tenantId) return; void (async () => { const { data } = await supabase.from("tenants").select("tenant_type").eq("id", user.tenantId).maybeSingle(); setIsCentralTenant(data?.tenant_type === "central"); })(); }, [user?.tenantId]);
