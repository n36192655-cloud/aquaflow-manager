...
export const Route = createFileRoute("/account")({
  head: () => ({ meta: [{ title: "حسابي — منصة ميزان" }] }),
...
function AccountPage() {
  ...
  async function updateRecoveryEmail() {
    ...
    if (!next || next.endsWith("@mizan.local") || !next.includes("@"))
      return toast.error("أدخل بريداً إلكترونياً حقيقياً للاسترداد");
    setEmailBusy(true);
    try {
  ...
  async function submit() {
    ...
    try {
      ...
      setConfirm("");
      toast.success(
        "تم تغيير كلمة المرور بنجاح. تم تسجيل الخروج من الجلسات السابقة، سجّل الدخول مجدداً.",
      );
    ...
    <div className="max-w-xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          إدارة بيانات الدخول الشخصية — منصة ميزان لإستدامة خدمات المياه
        </p>
      </div>
      <Card>
        <CardHeader>
        ...
        <CardContent className="space-y-2 text-sm">
          <div>
            ...
            <strong>{user?.username ?? "غير مُعيّن"}</strong>
          </div>
          <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>بريد الاسترداد</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-xs text-muted-foreground">
            الحسابات التي أنشأها المشرف تبدأ بمعرّف داخلي. أضف بريداً حقيقياً وفعّله حتى تستطيع
            استخدام «نسيت كلمة المرور» لاحقاً.
          </p>
          <Input
            type="email"
            autoComplete="email"
          ...
          >
            {emailBusy ? "جارٍ الحفظ…" : "حفظ بريد الاسترداد"}
          </Button>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent className="space-y-4">
          {user?.mustChangePassword && (
            <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">
              هذه كلمة مرور أولية صادرة من مالك المنصة. يجب تغييرها قبل استخدام النظام.
            </div>
          )}
          <div>
          ...
          <p className="text-xs text-muted-foreground">
            لا تُخزن منصة ميزان كلمة المرور في قاعدة بياناتها؛ Supabase Auth هو المسؤول عن
            credential.
          </p>
          <Button
            onClick={() => void submit()}
