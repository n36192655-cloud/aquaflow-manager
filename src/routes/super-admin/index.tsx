  ...
  return <div className="min-h-screen bg-background px-4 py-8" dir="rtl">
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <Card className="border-primary/20"><CardHeader><CardTitle className="flex items-center gap-2"><Network className="w-5 h-5" /> الهيكل المركزي</CardTitle></CardHeader><CardContent>
        {central ? <div className="rounded-lg border p-4"><div className="font-bold">{central.name}</div><div className="text-xs text-muted-foreground mt-1">مستأجر مركزي للإشراف</div><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={provisioningTenantId === central.id} onClick={() => void provisionUsers(central)}>{provisioningTenantId === central.id ? "جارٍ إنشاء الحسابات…" : "إنشاء الحسابات الناقصة"}</Button><Button size="sm" variant="outline" onClick={() => void setStatus(central)}>{central.subscription_status === "active" ? "تعليق المستأجر المركزي" : "تفعيل المستأجر المركزي"}</Button></div>{credentialSets[central.id]?.length > 0 && <div className="mt-3 rounded-md border border-primary/30 bg-primary/5 p-3 space-y-2"><div className="font-semibold text-sm">بيانات الحسابات الجديدة — تُعرض مرة واحدة</div><div className="text-xs text-muted-foreground">لا تُحفظ كلمات المرور في قاعدة بيانات ميزان ولا يمكن استرجاعها لاحقاً.</div>{credentialSets[central.id].map((u) => <div key={u.username} className="grid gap-1 rounded border bg-background p-2 text-sm"><div><span className="text-muted-foreground">الدور: </span>{u.displayName}</div><div dir="ltr" className="font-mono"><span className="text-muted-foreground">username: </span>{u.username}</div><div dir="ltr" className="font-mono"><span className="text-muted-foreground">password: </span>{u.password}</div></div>)}</div>}<div className="mt-3 grid gap-2">{projects.map((p) => <div key={p.id} className="flex items-center justify-between rounded-md bg-muted/40 px-3 py-2"><span className="font-medium">↳ {p.name}</span><Badge>{p.subscription_status === "active" ? "نشط" : p.subscription_status === "suspended" ? "موقوف" : "منتهي"}</Badge></div>)}{projects.length === 0 && <div className="text-sm text-muted-foreground">لا توجد مشاريع مرتبطة بعد.</div>}</div></div> : <div className="space-y-3"><p className="text-sm text-muted-foreground">لم يُنشأ المستأجر المركزي بعد.</p><div className="flex gap-2"><Input value={centralName} onChange={(e) => setCentralName(e.target.value)} /><Button onClick={() => void createCentral()}><Plus className="w-4 h-4 ml-1" /> إنشاء مركزي</Button></div></div>}
      </CardContent></Card>
