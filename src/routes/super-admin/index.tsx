...
function SuperAdminDashboard() {
  ...
  const [credentialSets, setCredentialSets] = useState<
    Record<string, Array<{ username: string; password: string; role: string; displayName: string }>>
  >({});
  ...
  async function refresh() {
    ...
    setLoading(false);
    if (error) {
      toast.error("تعذّر جلب المستأجرين");
      return;
    }
    ...
    const [{ data: roles }, { data: profiles }] = await Promise.all([
      supabase
        .from("user_roles")
        .select("user_id,tenant_id,role,must_change_password")
    ...
    for (const role of roles ?? []) {
      ...
      if (!profile || !role.tenant_id || !["manager", "collector", "reader"].includes(role.role))
        continue;
      (grouped[role.tenant_id] ??= []).push({
        userId: role.user_id,
        username: profile.username ?? "غير معيّن",
        displayName: profile.display_name ?? "مستخدم",
        role: role.role as TenantUser["role"],
        mustChangePassword: role.must_change_password === true,
      });
    }
  ...
  async function createCentral() {
    const { data: tenantId, error } = await supabase.rpc("create_central_tenant", {
      _name: centralName.trim(),
    });
    if (error || !tenantId) {
      toast.error(
        error?.message?.includes("already exists")
          ? "يوجد مستأجر مركزي بالفعل"
          : "تعذّر إنشاء المستأجر المركزي",
      );
      return;
    }
    ...
    try {
      ...
      if (result.credentials.length === 3)
        toast.success("تم إنشاء حسابات المدير والمحصل وقارئ العدادات للمستأجر المركزي");
    } catch {
  ...
  async function createProject() {
    if (!name.trim()) return;
    const { data: tenantId, error } = await supabase.rpc("create_project_tenant", {
      _name: name.trim(),
    });
    if (error || !tenantId) {
      toast.error("تعذّر إنشاء مشروع المياه");
      return;
    }
    setName("");
    toast.success("تم إنشاء مشروع المياه وربطه بالإشراف المركزي");
    await refresh();
    try {
      const result = await provisionTenantUsers({ data: { tenantId } });
      setCredentialSets((prev) => ({ ...prev, [tenantId]: result.credentials }));
  ...
  async function provisionUsers(t: TenantRow) {
    ...
    try {
      ...
      if (result.credentials.length === 0)
        toast.info(
          "الحسابات الثلاثة موجودة بالفعل لهذا المشروع؛ لا يمكن عرض كلمات مرورها الحالية.",
        );
      else
        toast.success(
          `تم إنشاء ${result.credentials.length} حسابات جديدة. كلمات المرور تظهر الآن فقط.`,
        );
    } catch (error) {
  ...
  async function resetPassword(t: TenantRow, user: TenantUser) {
    try {
      const result = await resetTenantUserPassword({
        data: { tenantId: t.id, userId: user.userId },
      });
  ...
  return (
    <div className="min-h-screen bg-background px-4 py-8" dir="rtl">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <Card className="border-primary/20">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Network className="w-5 h-5" /> الهيكل المركزي
            </CardTitle>
          </CardHeader>
          <CardContent>
            {central ? (
              <div className="rounded-lg border p-4">
                <div className="font-bold">{central.name}</div>
                <div className="text-xs text-muted-foreground mt-1">مستأجر مركزي للإشراف</div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={provisioningTenantId === central.id}
                    onClick={() => void provisionUsers(central)}
                  >
                    {provisioningTenantId === central.id
                      ? "جارٍ إنشاء الحسابات…"
                      : "إنشاء الحسابات الناقصة"}
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => void setStatus(central)}>
                    {central.subscription_status === "active"
                      ? "تعليق المستأجر المركزي"
                      : "تفعيل المستأجر المركزي"}
                  </Button>
                </div>
                {credentialSets[central.id]?.length > 0 && (
                  <div className="mt-3 rounded-md border border-primary/30 bg-primary/5 p-3 space-y-2">
                    <div className="font-semibold text-sm">
                      بيانات الحسابات الجديدة — تُعرض مرة واحدة
                    </div>
                    <div className="text-xs text-muted-foreground">
                      لا تُحفظ كلمات المرور في قاعدة بيانات ميزان ولا يمكن استرجاعها لاحقاً.
                    </div>
                    {credentialSets[central.id].map((u) => (
                      <div
                        key={u.username}
                        className="grid gap-1 rounded border bg-background p-2 text-sm"
                      >
                        <div>
                ...
                <div className="mt-3 grid gap-2">
                  ...
                  {projects.length === 0 && (
                    <div className="text-sm text-muted-foreground">لا توجد مشاريع مرتبطة بعد.</div>
                  )}
                </div>
              </div>
            ) : (
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">لم يُنشأ المستأجر المركزي بعد.</p>
                <div className="flex gap-2">
                  <Input value={centralName} onChange={(e) => setCentralName(e.target.value)} />
                  <Button onClick={() => void createCentral()}>
                    <Plus className="w-4 h-4 ml-1" /> إنشاء مركزي
                  </Button>
                </div>
              </div>
            )}
        ...
        <Card>
          <CardHeader>
            <CardTitle>إنشاء مشروع مياه</CardTitle>
          </CardHeader>
          <CardContent className="flex gap-2">
            <Input
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="مثال: مشروع مياه المعافر"
            />
            <Button onClick={() => void createProject()} disabled={!central || !name.trim()}>
              إنشاء وربط
            </Button>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>المشاريع ({projects.length})</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={t.subscription_status !== "active" || provisioningTenantId === t.id}
                    onClick={() => void provisionUsers(t)}
                  >
                    {provisioningTenantId === t.id
                      ? "جارٍ إنشاء الحسابات…"
                      : "إنشاء الحسابات الناقصة"}
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => void setStatus(t)}>
                ...
                <div className="mt-3 grid gap-2 md:grid-cols-3">
                  {(tenantUsers[t.id] ?? []).map((u) => (
                    <div key={u.userId} className="rounded-md border p-3 text-sm">
                      ...
                      <div dir="ltr" className="mt-1 font-mono text-xs">
                        {u.username}
                      </div>
                      <div className="mt-1 text-xs text-muted-foreground">
                        {u.mustChangePassword ? "كلمة مرور مؤقتة — يجب تغييرها" : "حساب مفعّل"}
                      </div>
                      <Button
                        size="sm"
                        variant="outline"
                        className="mt-2 w-full"
                        onClick={() => void resetPassword(t, u)}
                      >
                ...
                {credentialSets[t.id]?.length > 0 && (
                  <div className="mt-3 rounded-md border border-primary/30 bg-primary/5 p-3 space-y-2">
                    <div className="font-semibold text-sm">
                      بيانات الحسابات الجديدة — تُعرض مرة واحدة
                    </div>
                    <div className="text-xs text-muted-foreground">
                      لا تُحفظ كلمات المرور في قاعدة بيانات ميزان ولا يمكن استرجاعها لاحقاً. سلّمها
                      للعميل ثم اطلب منه إضافة بريد استرداد من صفحة «حسابي».
                    </div>
                    {credentialSets[t.id].map((u) => (
                      <div
                        key={u.username}
                        className="grid gap-1 rounded border bg-background p-2 text-sm"
                      >
            ...
            {projects.length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-6">لا توجد مشاريع.</p>
            )}
          </CardContent>
        </Card>
