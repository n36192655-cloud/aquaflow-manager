...
type Bill = { id: string; customer_id: string; reading_id: string | null; subtotal: number; arrears: number; total: number; status: string; issued_at: string; tariff_category?: string | null; consumption_lpd?: number | null; tariff_breakdown?: unknown };
...
function BillsPage() {
  ...
  async function load() {
    ...
    const [b, c, m, r, p] = await Promise.all([
      supabase.from("water_bills").select("id,customer_id,reading_id,subtotal,arrears,total,status,issued_at,tariff_category,consumption_lpd,tariff_breakdown").eq("tenant_id", user.tenantId).order("issued_at", { ascending: false }),
