...
import { Printer, Wallet, Droplets, Smartphone, ShieldCheck } from "lucide-react";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          إصدار تلقائي عند اعتماد القراءة — تشمل بند «متأخرات سابقة»
        </p>
      </div>
              ...
              {filtered.map((b) => {
                ...
                return (
                  <TableRow key={b.id}>
                    ...
                    <TableCell>
                      {b.arrears > 0 ? (
                        <span className="text-destructive font-semibold">{fmtYER(b.arrears)}</span>
                      ) : (
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        <Button size="sm" variant="outline" onClick={() => setPrintBill(b.id)}>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Select value={method} onValueChange={(v: "نقدي" | "الكريمي") => setMethod(v)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="نقدي">نقدي (يتم استلامه من الميدان)</SelectItem>
                  <SelectItem value="الكريمي">تحويل بنكي — الكريمي</SelectItem>
                </SelectContent>
              </Select>
            </div>
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="rounded-lg border p-4 bg-gradient-to-br from-primary/5 to-primary/0 space-y-3 text-sm">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="text-muted-foreground">المشترك</div>
            <div className="font-semibold">{c?.name}</div>
            <div className="text-muted-foreground">حساب السداد</div>
            <div className="font-mono" dir="ltr">
              {c?.pay_account}
            </div>
            <div className="text-muted-foreground">فاتورة</div>
            <div className="font-mono">{b.serial}</div>
            <div className="text-muted-foreground">المستحق</div>
            <div className="font-bold text-lg">{fmtYER(remaining)}</div>
          </div>
          {step === "otp" && (
          ...
          {step === "done" && (
            <div className="pt-2 text-center text-emerald-600 font-semibold">
              ✓ تم إرسال إشعار التحويل — بانتظار اعتماد الإدارة
            </div>
          )}
        </div>
        <DialogFooter>
          ...
          {step === "otp" && (
            <Button
              onClick={() => {
                ...
                addPayment({ billId: b.id, amount: remaining, method: "الكريمي", by: user?.name });
                setStep("done");
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{b.serial}</DialogTitle>
        </DialogHeader>
        <div id="printable" className="p-6 bg-white text-slate-900 rounded-lg border">
          <div className="text-center border-b pb-3 mb-4">
            <div className="text-2xl font-bold" style={{ color: "var(--water)" }}>
              ميزان
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              <div className="text-slate-500 text-xs">المشترك</div>
              <div className="font-semibold">{c?.name}</div>
            </div>
            <div>
              <div className="text-slate-500 text-xs">الهاتف</div>
              <div dir="ltr" className="text-right">
                {c?.phone}
              </div>
            </div>
            <div>
              <div className="text-slate-500 text-xs">المديرية</div>
              <div>{c?.directorate ?? "—"}</div>
            </div>
            <div>
              <div className="text-slate-500 text-xs">رقم العداد</div>
              <div className="font-mono">{m?.number}</div>
            </div>
            <div>
              <div className="text-slate-500 text-xs">القراءة السابقة</div>
              <div>{r?.previous}</div>
            </div>
            <div>
              <div className="text-slate-500 text-xs">القراءة الحالية</div>
              <div>{r?.current}</div>
            </div>
            <div className="col-span-2">
              <div className="text-slate-500 text-xs">الاستهلاك</div>
              <div className="font-semibold">{r?.consumption} م³</div>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t space-y-1 text-sm">
            <div className="flex justify-between">
              <span className="text-slate-500">استهلاك الشهر</span>
              <span>{fmtYER(b.subtotal)}</span>
            </div>
            {b.arrears > 0 && (
              <div className="flex justify-between text-red-700">
                <span>متأخرات سابقة</span>
                <span>{fmtYER(b.arrears)}</span>
              </div>
            )}
        ...
        <DialogFooter>
          <Button onClick={() => window.print()}>
            <Printer className="w-4 h-4 ms-1" /> طباعة / PDF
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
