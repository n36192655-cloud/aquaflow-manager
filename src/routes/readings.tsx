...
import {
  approveFieldReading,
  createMeterReadingImageUrl,
  newClientId,
  recordFieldReading,
  rejectFieldReading,
  removeMeterReadingImage,
...
function ReadingsPage() {
  ...
  async function loadData() {
    ...
    const [customerResult, meterResult, profileResult, readingResult, billResult] =
      await Promise.all([
        supabase
          .from("customers")
        ...
        supabase
          .from("meters")
          .select("id,customer_id,serial_number,status,profile_id")
          .eq("tenant_id", user.tenantId)
          .order("serial_number"),
        supabase
          .from("meter_profiles")
        ...
        supabase
          .from("water_readings")
          .select(
            "id,meter_id,customer_id,previous,current_reading,consumption,status,verification_status,photo_url,ocr_serial,ocr_confidence,lat,lng,review_reason,created_at",
          )
          .eq("tenant_id", user.tenantId)
          .order("created_at", { ascending: false }),
        supabase
          .from("water_bills")
          .select("id,customer_id,reading_id,total,arrears,status,issued_at")
          .eq("tenant_id", user.tenantId)
          .order("issued_at", { ascending: false }),
      ]);
    const firstError = [customerResult, meterResult, profileResult, readingResult, billResult].find(
      (x) => x.error,
    )?.error;
  ...
  async function refreshOffline() {
    try {
      setPendingOffline(await getPending());
    } catch (error) {
      console.error(error);
    }
  }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`mizan-readings-${user?.tenantId ?? "none"}`)
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  const filteredMeters = useMemo(() => {
    ...
    return meters.filter((meter) => {
      ...
      return (
        meter.serial_number.toLowerCase().includes(q) || customer?.name.toLowerCase().includes(q)
      );
    });
  ...
  const selectedCustomer = selected
    ? customers.find((customer) => customer.id === selected.customer_id)
    : null;
  const selectedProfile = selected?.profile_id
    ? profiles.find((profile) => profile.id === selected.profile_id)
    : null;
  const approvedHistory = selected
    ? readings.filter(
        (reading) => reading.meter_id === selected.id && reading.verification_status === "approved",
      )
    : [];
  ...
  function handleOcr(result: OcrResult) {
    ...
    if (result.serialMatch === "mismatch")
      toast.error(`رفض بصري: ${result.serial} لا يطابق ${selected?.serial_number}`);
    else if (result.confidence < 0.7 || result.reading == null)
      toast.warning("الثقة منخفضة؛ راجع القراءة وأدخلها يدوياً. لا يتم التخمين.");
  }
  ...
  async function captureGeo() {
    ...
    try {
      ...
      toast.error(`فشل تحديد الموقع: ${(error as Error).message}`);
    } finally {
      setGeoBusy(false);
    }
  }
  ...
  async function saveReading() {
    ...
    if (!fix) {
      try {
        ...
        if (isReader) return toast.error(`الموقع مطلوب: ${(error as Error).message}`);
      }
    }
    ...
    try {
      if (photo)
        uploadedPath = await uploadMeterReadingImage(photo, user.tenantId, user.userId, clientId);
      ...
      toast.success(
        `تم تسجيل القراءة #${result.reading_id} بانتظار اعتماد المدير. لم تُصدر فاتورة قبل الاعتماد.`,
      );
      resetForm();
      await loadData();
    } catch (error) {
      ...
      if (isTransientError(error)) {
        ...
        await refreshOffline();
        toast.warning(
          "تعذر الوصول إلى الخادم الآن؛ حُفظت القراءة محلياً ولم تُعتبر مسجلة حتى تتم المزامنة.",
        );
      } else {
  ...
  async function approve(readingId: string) {
    setBusy(true);
    try {
      await approveFieldReading(readingId);
      toast.success("تم اعتماد القراءة وإصدار الفاتورة ذرّياً");
      await loadData();
    } catch (error) {
      toast.error(`فشل الاعتماد: ${(error as Error).message}`);
    } finally {
      setBusy(false);
    }
  }
  ...
  async function reject(readingId: string) {
    ...
    setBusy(true);
    try {
      await rejectFieldReading(readingId, reason);
      toast.info("تم رفض القراءة وتسجيل السبب");
      await loadData();
    } catch (error) {
      toast.error(`فشل الرفض: ${(error as Error).message}`);
    } finally {
      setBusy(false);
    }
  }
  ...
  async function retryOffline() {
    ...
    try {
      ...
      if (result.failed)
        toast.warning(`فشلت مزامنة ${result.failed} قراءة؛ بقيت في الطابور مع سبب الخطأ`);
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            مسار إنتاجي: تحقق → GPS → خادم → اعتماد → فاتورة
          </p>
        </div>
        <div className="flex items-center gap-2">
          {!online && (
            <Badge variant="destructive">
              <WifiOff className="w-3 h-3 ms-1" /> أوفلاين
            </Badge>
          )}
          <Button size="sm" variant="outline" onClick={() => void retryOffline()} disabled={busy}>
            <RefreshCw className="w-4 h-4 ms-1" /> مزامنة
          </Button>
        </div>
      </div>
          ...
          <CardContent className="space-y-4">
            {meters.length === 0 && (
              <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900 flex gap-2">
                ...
                <div>
                  <b>لا توجد عدادات فعلية مرتبطة بالمشروع في قاعدة البيانات.</b>
                  <div className="mt-1">
                    لن يتم إنشاء قراءة وهمية أو استخدام بيانات الاختبار. يجب إدخال العدادات الحقيقية
                    عبر مسار الإدارة قبل بدء القراءة الميدانية.
                  </div>
                </div>
              </div>
            )}
            <div>
              <Label>بحث المشترك / رقم العداد</Label>
              <Input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="اكتب اسم المشترك أو رقم العداد"
                className="mt-1"
              />
            </div>
            <div>
              ...
              >
                <option value="">اختر عداداً فعّالاً</option>
                {filteredMeters
                  .filter((m) => m.status === "active")
                  .map((meter) => {
                    const customer = customers.find((c) => c.id === meter.customer_id);
                    return (
                      <option key={meter.id} value={meter.id}>
                        {customer?.name ?? "مشترك غير معروف"} — {meter.serial_number}
                      </option>
                    );
                  })}
              </select>
            ...
            {selected && selectedCustomer && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 rounded-lg border bg-muted/30 p-3 text-xs">
                <Info label="المشترك">{selectedCustomer.name}</Info>
                <Info label="الهاتف">{selectedCustomer.phone ?? "—"}</Info>
                <Info label="رقم العداد">
                  <span dir="ltr" className="font-mono">
                    {selected.serial_number}
                  </span>
                </Info>
                <Info label="القراءة السابقة">
                  <span dir="ltr" className="font-mono">
                    {previous}
                  </span>
                </Info>
            ...
            {selectedProfile && (
              <div className="text-xs text-muted-foreground">
                نوع العداد: {selectedProfile.display_type} · الأرقام الصحيحة:{" "}
                {selectedProfile.integer_digits || "غير محدد"} · الكسور:{" "}
                {selectedProfile.decimal_digits}
              </div>
            )}
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              {photo && (
                <Badge variant="outline">
                  <ImageIcon className="w-3 h-3 ms-1" /> صورة
                </Badge>
              )}
              {ocrSerial && (
                <Badge
                  variant={
                    selected &&
                    normalizeSerial(ocrSerial) === normalizeSerial(selected.serial_number)
                      ? "default"
                      : "destructive"
                  }
                >
                  <ShieldAlert className="w-3 h-3 ms-1" /> OCR: {ocrSerial}
                </Badge>
              )}
              {ocrConfidence != null && (
                <Badge variant="outline">ثقة OCR: {(ocrConfidence * 100).toFixed(0)}%</Badge>
              )}
              {geo && (
                <Badge variant="outline">
                  <MapPin className="w-3 h-3 ms-1" />
                  {geo.accuracy.toFixed(0)}م
                </Badge>
              )}
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <WifiOff className="w-4 h-4" /> طابور الأوفلاين ({pendingOffline.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {pendingOffline.map((item) => (
              ...
              >
                <div>
                  <div className="font-mono">{item.clientId}</div>
                  <div className="text-muted-foreground">
                    قراءة {item.current} · محاولات {item.retryCount} · {item.state}
                  </div>
                  {item.lastError && <div className="text-destructive mt-1">{item.lastError}</div>}
                </div>
                <Button
                  size="sm"
                  variant="outline"
...
function PendingApprovals({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>قراءات بانتظار الاعتماد ({readings.length})</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {readings.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-6">لا توجد قراءات معلقة.</p>
        )}
        {readings.map((reading) => {
          const meter = meters.find((m) => m.id === reading.meter_id);
          const customer = customers.find((c) => c.id === reading.customer_id);
          const bill = bills.find((b) => b.reading_id === reading.id);
          return (
            <PendingRow
              key={reading.id}
              reading={reading}
...
function PendingRow({
  ...
  useEffect(() => {
    if (reading.photo_url) void createMeterReadingImageUrl(reading.photo_url).then(setImageUrl);
  }, [reading.photo_url]);
  return (
    <div className="rounded-lg border p-3 grid md:grid-cols-[120px_1fr_auto] gap-3 items-start">
      <div className="w-full h-24 rounded-md border bg-muted/30 grid place-items-center overflow-hidden">
        {imageUrl ? (
          <img src={imageUrl} alt="صورة العداد" className="w-full h-full object-cover" />
        ) : (
      ...
      <div className="text-xs space-y-1">
        <div className="font-semibold">
          {customer?.name ?? "مشترك"} —{" "}
          <span className="font-mono">{meter?.serial_number ?? "—"}</span>
        </div>
        <div>
          القراءة: {reading.previous} → <b>{reading.current_reading}</b> · الاستهلاك:{" "}
          {reading.consumption}
        </div>
        <div>
          OCR: {reading.ocr_serial ?? "غير متوفر"} · الثقة:{" "}
          {reading.ocr_confidence == null ? "—" : `${(reading.ocr_confidence * 100).toFixed(0)}%`}
        </div>
        <div>
          GPS:{" "}
          {reading.lat == null || reading.lng == null
            ? "غير متوفر"
            : `${reading.lat.toFixed(5)}, ${reading.lng.toFixed(5)}`}
        </div>
        {bill && <div>فاتورة موجودة: {bill.total}</div>}
      </div>
      <div className="flex md:flex-col gap-2">
        <Button size="sm" onClick={() => onApprove(reading.id)} disabled={busy}>
          <Check className="w-3 h-3 ms-1" /> اعتماد
        </Button>
        <Button
          size="sm"
          variant="destructive"
          onClick={() => onReject(reading.id)}
          disabled={busy}
        >
...
function History({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>سجل القراءات ({readings.length})</CardTitle>
      </CardHeader>
      <CardContent className="space-y-2">
        {readings.length === 0 && (
          <p className="text-sm text-muted-foreground py-6 text-center">لا توجد قراءات.</p>
        )}
        {readings.map((reading) => {
          const meter = meters.find((m) => m.id === reading.meter_id);
          const customer = customers.find((c) => c.id === reading.customer_id);
          return (
            <div
              key={reading.id}
              className="rounded-md border p-3 grid md:grid-cols-5 gap-2 text-xs"
            >
              <div>{customer?.name ?? "—"}</div>
              <div className="font-mono">{meter?.serial_number ?? reading.id}</div>
              <div>
                {reading.previous} → {reading.current_reading}
              </div>
              <div>{reading.consumption} م³</div>
              <div>
                <Badge
                  variant={
                ...
                {reading.review_reason && (
                  <div className="text-destructive mt-1">{reading.review_reason}</div>
                )}
              </div>
            </div>
          );
