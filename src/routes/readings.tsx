...
function ReadingsPage() {
  const { meters, readings, customers, bills, addReadingWithBill, approveReading, rejectReading } =
    useStore();
  const { user } = useAuth();
  ...
  const last = selected
    ? readings
        .filter((r) => r.meter_id === selected.id)
  ...
  function handleOcr(res: OcrResult) {
    ...
    if (res.serialMatch === "mismatch") {
      toast.error(
        `عدم تطابق: العداد الملتقط ${res.serial} لا يطابق العداد المسجل ${selected?.number}`,
      );
      return;
    }
    if (res.serialMatch === "match") toast.success(`تطابق ✓ رقم العداد ${res.serial}`);
    else if (res.reading != null)
      toast.info(`تم التقاط قراءة ${res.reading} — رقم العداد غير مقروء بوضوح`);
  }
  ...
  async function saveReading() {
    ...
    if (
      ...
      ocrSerial.replace(/[-\s]/g, "").toUpperCase() !==
        selected.number.replace(/[-\s]/g, "").toUpperCase()
    ) {
      return toast.error(
        `رفض: رقم العداد الملتقط (${ocrSerial}) لا يطابق المسجل (${selected.number})`,
      );
    }
    ...
    if (!online) {
      ...
      toast.warning(
        "🔒 وضع الأوفلاين: تم حفظ القراءة محلياً مع تثبيت إحداثيات الـ GPS بنجاح. ستُزامَن تلقائياً فور توفر الشبكة.",
      );
      resetForm();
      return;
    }
    ...
    else if (reading.flag === "suspicious")
      toast.warning("قراءة مشبوهة (تفوق 3× المتوسط) — أُرسلت للاعتماد");
    else if (bill)
      toast.success(`تم — استهلاك ${reading.consumption} • فاتورة ${bill.serial} بانتظار الاعتماد`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {isReader
              ? "ابحث عن المشترك، صوّر العداد، وسجّل القراءة"
              : "إدخال قراءات جديدة ومراجعتها قبل الاعتماد"}
          </p>
        </div>
        {!online && (
            ...
            {selected && selectedCustomer && (
              <div className="rounded-lg border p-3 bg-muted/30 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                ...
                <Info label="القراءة السابقة">
                  <span className="font-mono">{last?.current ?? 0}</span>
                </Info>
              </div>
            )}
              ...
              <div className="md:col-span-2">
                ...
                {selected && current !== "" && +current > (last?.current ?? 0) && (
                  <p className="text-[11px] text-muted-foreground mt-1">
                    الاستهلاك المتوقع: {+current - (last?.current ?? 0)} —{" "}
                    {fmtYER(priceFor(selected.type, +current - (last?.current ?? 0)))}
                  </p>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-2 text-xs">
                {photo && (
                  <Badge variant="outline" className="gap-1">
                    <ImageIcon className="w-3 h-3" /> صورة ملتقطة
                  </Badge>
                )}
                {ocrSerial && (
                  <Badge
                ...
                {geo && (
                  <Badge variant="outline" className="gap-1">
                    <MapPin className="w-3 h-3" /> {geo.lat.toFixed(4)}, {geo.lng.toFixed(4)}
                  </Badge>
                )}
              </div>
            )}
      ...
      {tab === "pending" && !isReader && (
        <Card>
          <CardHeader>
            <CardTitle>قراءات بانتظار الاعتماد ({pending.length})</CardTitle>
          </CardHeader>
          <CardContent className="p-4 space-y-3">
            {pending.length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-6">لا يوجد قراءات معلقة</p>
            )}
            {pending.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  className="rounded-lg border p-3 grid md:grid-cols-[110px_1fr_auto] gap-3 items-start"
                >
                  {r.photo ? (
                    <img
                      src={r.photo}
                  ...
                  <div className="text-xs space-y-1">
                    <div className="flex items-center gap-2 text-sm font-semibold">
                      {c?.name} — <span className="font-mono">{m?.number}</span>
                    </div>
                    <div className="text-muted-foreground">
                      قراءة {r.previous} →{" "}
                      <span className="text-foreground font-mono">{r.current}</span> · استهلاك{" "}
                      {r.consumption}
                    </div>
                    {b && (
                      <div>
                        الفاتورة: <span className="font-mono">{b.serial}</span> · {fmtYER(b.total)}{" "}
                        {b.arrears > 0 && (
                          <span className="text-destructive">(متأخرات {fmtYER(b.arrears)})</span>
                        )}
                      </div>
                    )}
                    <div className="flex gap-2 flex-wrap text-[11px] text-muted-foreground">
                      {r.ocr_serial && (
                        <span>
                          OCR: <span className="font-mono">{r.ocr_serial}</span>
                        </span>
                      )}
                      {r.lat != null && r.lng != null && (
                        <a
                          ...
                          rel="noreferrer"
                        >
                          <MapPin className="inline w-3 h-3" /> {r.lat.toFixed(4)},{" "}
                          {r.lng.toFixed(4)}
                        </a>
                      )}
                        ...
                        <TableCell>
                          {r.status === "pending" ? (
                            ...
                            <Badge variant="destructive">
                              <X className="w-3 h-3 ms-1" /> مرفوضة
                            </Badge>
                          ) : r.flag === "ok" ? (
                            <Badge variant="outline" className="gap-1 text-water">
                              <CheckCircle2 className="w-3 h-3" /> معتمدة
                            </Badge>
                          ) : r.flag === "suspicious" ? (
                            <Badge variant="secondary" className="gap-1">
                              <AlertCircle className="w-3 h-3" /> مشبوهة
                            </Badge>
                          ) : (
                            <Badge variant="destructive" className="gap-1">
                              <AlertCircle className="w-3 h-3" /> خاطئة
                            </Badge>
                          )}
                        </TableCell>
                      </TableRow>
