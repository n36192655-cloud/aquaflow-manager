...
function LoginPage() {
  ...
  function selectAccount(account: LoginPreset) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (u) {
        ...
        navigate({ to: u.isSuperAdmin ? "/super-admin" : defaultRouteFor(u.role), replace: true });
      }
    } finally {
      setBusy(false);
    }
    ...
    <div className="flex-1 grid place-items-center px-4 py-10">
      <Card className="w-full max-w-lg shadow-xl">
        <CardHeader className="text-center">
          <div className="mx-auto w-14 h-14 rounded-2xl grid place-items-center mb-2 bg-water"><Droplets className="w-7 h-7 text-white" /></div>
          <CardTitle className="text-2xl">منصة ميزان لإستدامة خدمات المياه</CardTitle>
          <p className="text-xs text-muted-foreground mt-1">مشروع مياه المسراخ</p>
        </CardHeader>
        <CardContent className="space-y-5">
          <div>
            <Label className="mb-2 block">اختر الحساب</Label>
          ...
          {mounted && licStatus !== "active" && <p className="text-xs text-destructive text-center">{statusLabel(licStatus)} — تواصل مع مزوّد الخدمة</p>}
        </CardContent>
      </Card>
    </div>
    <CopyrightFooter />
