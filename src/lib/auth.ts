...
export interface AuthUser { name: string; role: Role; seatId?: string; userId?: string; tenantId?: string; isSuperAdmin?: boolean; }
interface AuthState { user: AuthUser | null; loginError: LicenseStatus | "bad_credentials" | "not_configured" | null; login: (email: string, password: string) => Promise<boolean>; logout: () => void; heartbeat: () => void; hydrateFromSupabase: () => Promise<void>; }
export const useAuth = create<AuthState>()(persist((set) => ({
  user: null, loginError: null,
  login: async (email, password) => {
    if (!email.trim() || !password) { set({ loginError: "bad_credentials" }); return false; }
    try {
      ...
      const lic = useLicense.getState(); lic.initIfNeeded(); const seat = lic.acquireSeat(u.userId ?? email, u.role);
  ...
  hydrateFromSupabase: async () => {
    ...
    const { data: profile, error: profileError } = await supabase.from("profiles").select("tenant_id, display_name").eq("id", user.id).maybeSingle(); if (profileError) throw profileError;
    ...
    set({ user: { name: profile?.display_name ?? user.email ?? "مستخدم", role, userId: user.id, tenantId: profile?.tenant_id ?? undefined, isSuperAdmin }, loginError: null });
  },
