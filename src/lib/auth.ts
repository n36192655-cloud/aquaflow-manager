...
export const useAuth = create<AuthState>()(persist((set) => ({
  ...
  login: async (email, password) => {
    ...
    try {
      ...
      const lic = useLicense.getState(); lic.initIfNeeded();
      const seat = lic.acquireSeat(u.userId ?? email, u.role);
  ...
  hydrateFromSupabase: async () => {
    ...
    const { data: profile, error: profileError } = await supabase.from("profiles").select("tenant_id, display_name").eq("id", user.id).maybeSingle();
