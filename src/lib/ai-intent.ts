...
async function subscriberLedger(tenantId: string, text: string): Promise<AiResponse> {
  ...
  if (!query) {
    return {
      kind: "suggestions",
      text: "حدد المشترك بالاسم أو رقم الهاتف أو رقم حساب السداد.",
      suggestions: SUGGESTIONS,
    };
  }
  ...
  const [byName, byPhone, byAccount] = await Promise.all([
    supabase
      .from("customers")
      .select("id,name,phone,address,pay_account")
  ...
  if (uniqueCustomers.length === 0) {
    return {
      kind: "suggestions",
      text: "لم أجد مشتركاً مطابقاً في قاعدة البيانات الحالية. لم يتم اختراع نتيجة.",
      suggestions: SUGGESTIONS,
    };
  }
  ...
  const arrears = bills.reduce(
    (sum, bill) => sum + Math.max(0, Number(bill.total) - (paidByBill.get(bill.id) ?? 0)),
    0,
  );
  ...
  const readingsResult = readingIds.length
    ? await supabase
        .from("water_readings")
        .select("id,consumption,created_at")
        .eq("tenant_id", tenantId)
        .in("id", readingIds)
    : { data: [], error: null };
  ...
  const series = bills.slice(-12).map((bill) => ({
    label: new Intl.DateTimeFormat("ar-YE", { month: "short", day: "numeric" }).format(
      new Date(bill.issued_at),
    ),
...
async function lossAnalysis(tenantId: string, text: string): Promise<AiResponse> {
  ...
  const [productionResult, readingsResult] = await Promise.all([
    supabase
      ...
      .select("production_m3,recorded_at,verification_status")
      .eq("tenant_id", tenantId)
      .gte("recorded_at", start)
      .lt("recorded_at", end),
    supabase
      .from("water_readings")
      .select("consumption,created_at,status,verification_status")
      .eq("tenant_id", tenantId)
  ...
  const production = (productionResult.data ?? [])
    .filter(
      (row) =>
        ...
        Number.isFinite(Number(row.production_m3)) &&
        Number(row.production_m3) > 0,
    )
  ...
  const consumed = (readingsResult.data ?? [])
    .filter(
      (row) =>
        row.verification_status === "approved" &&
        row.status === "approved" &&
        Number.isFinite(Number(row.consumption)) &&
  ...
  if (loss < 0)
    alerts.push(
      "تحذير جودة بيانات: الاستهلاك المعتمد يتجاوز مدخل المياه المعتمد؛ لم يتم إخفاء التناقض أو تحويله إلى صفر.",
    );
  if (pct != null && pct > 15)
    alerts.push(
      `الفاقد الحسابي ${pct.toFixed(1)}% ويتطلب تفسيراً ميدانياً قبل وصفه كتسرب أو فاقد فني.`,
    );
...
async function paymentStatus(tenantId: string): Promise<AiResponse> {
  const [billsResult, customersResult, paymentsResult] = await Promise.all([
    supabase
      .from("water_bills")
      .select("id,reading_id,total,status,customer_id")
      .eq("tenant_id", tenantId)
  ...
  for (const p of paymentsResult.data ?? []) {
    if (p.status === "approved")
      approvedByBill.set(p.bill_id, (approvedByBill.get(p.bill_id) ?? 0) + Number(p.amount));
  }
  ...
  const readingIds = (billsResult.data ?? [])
    .map((b) => b.reading_id)
    .filter((id): id is string => Boolean(id));
  const readingsResult = readingIds.length
    ? await supabase.from("water_readings").select("id,meter_number").in("id", readingIds)
    : { data: [], error: null };
  if (readingsResult.error) return genericDataError();
  ...
  for (const bill of billsResult.data ?? []) {
    ...
    if (balance <= 0 && paidAmount > 0)
      paid.push({
        id: bill.id,
        name,
        serial: meterNumbers.get(bill.reading_id ?? "") ?? "—",
        total,
      });
    else
      unpaid.push({
...
async function revenueReport(tenantId: string, text: string): Promise<AiResponse> {
  ...
  const cash = (paymentsResult.data ?? [])
    .filter((p) => p.method === "cash")
    .reduce((sum, p) => sum + Number(p.amount), 0);
  const bank = (paymentsResult.data ?? [])
    .filter((p) => p.method !== "cash")
    .reduce((sum, p) => sum + Number(p.amount), 0);
  ...
  return {
    kind: "revenue_report",
    range: {
      from: day(range.start.toISOString()),
      to: day(new Date(range.end.getTime() - 1).toISOString()),
      label: range.label,
    },
    totals: {
      cash,
      ...
      avg: paymentsResult.data?.length ? total / paymentsResult.data.length : 0,
    },
    series: [...days.entries()]
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([dayKey, value]) => ({ day: dayKey.slice(5), ...value })),
  };
}
...
export async function answerQuestion(q: string): Promise<AiResponse> {
  ...
  if (!text)
    return {
      kind: "suggestions",
      text: "اكتب سؤالك وسأبحث في بيانات المشروع الحالية فقط.",
      suggestions: SUGGESTIONS,
    };
  ...
  if (!tenantId) {
    return {
      kind: "text",
      text: "لا يوجد مشروع تشغيلي مرتبط بالحساب الحالي. لا توجد بيانات يمكن عرضها.",
      suggestions: SUGGESTIONS,
    };
  }
  ...
  try {
    ...
    if (
      ...
      text.includes("تسرب") ||
      text.includes("خسائر") ||
      text.includes("تحليل الفاقد")
    ) {
      return await lossAnalysis(tenantId, text);
    }
    if (
      text.includes("من دفع") ||
      text.includes("من لم يدفع") ||
      text.includes("المدفوع") ||
      text.includes("غير المدفوع") ||
      text.includes("حالة الدفع")
    ) {
    ...
    return {
      kind: "suggestions",
      text: "أستطيع الإجابة من بيانات قاعدة البيانات الحالية فقط. اختر نوع الاستعلام:",
      suggestions: SUGGESTIONS,
    };
  } catch {
