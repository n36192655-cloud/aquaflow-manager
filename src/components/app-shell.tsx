...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    let alive = true;
    void getCurrentTenant(user.tenantId)
      .then((t) => {
  ...
  useEffect(() => {
    ...
    if (user.isSuperAdmin) {
      setRemoteLicenseStatus("active");
      return () => {
        alive = false;
      };
    }
    if (!user.tenantId) {
      setRemoteLicenseStatus("invalid");
      return () => {
        alive = false;
      };
    }
  ...
  useEffect(() => {
    ...
    if (pathname !== "/subscription" && pathname !== "/login" && remoteLicenseStatus)
  ...
  useEffect(() => {
    ...
    if (!canAccess(user.role, pathname, user.isSuperAdmin))
      navigate({ to: defaultRouteFor(user.role, user.isSuperAdmin), replace: true });
  ...
  if (!user.isSuperAdmin && remoteLicenseStatus === null && pathname !== "/subscription")
    return (
      <div className="min-h-screen grid place-items-center bg-background text-foreground">
        <div className="text-sm text-muted-foreground">جارٍ التحقق من اشتراك المشروع…</div>
      </div>
    );
  if (!user.isSuperAdmin && remoteLicenseStatus !== "active" && pathname !== "/subscription")
    return (
      <div className="min-h-screen grid place-items-center bg-background text-foreground">
        <div className="text-center space-y-2">
          <div className="text-sm font-semibold">الوصول التشغيلي متوقف</div>
          <div className="text-xs text-muted-foreground">
            يجب أن يكون اشتراك المشروع نشطاً للوصول إلى بيانات التشغيل.
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen flex w-full bg-background text-foreground">
      <aside className="hidden md:flex w-64 shrink-0 flex-col bg-sidebar text-sidebar-foreground border-l border-sidebar-border">
        <div className="px-5 py-6 border-b border-sidebar-border">
          ...
          {tenant && (
            <div className="mt-4 rounded-lg bg-sidebar-accent/60 px-3 py-2">
              <div className="text-[10px] text-sidebar-foreground/60">المشروع الحالي</div>
              <div className="text-sm font-bold truncate" title={tenant.name}>
                {tenant.name}
              </div>
            </div>
          )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-card border-b px-4 py-2 flex items-center justify-between gap-2">
          <div className="md:hidden font-bold truncate">
            {tenant?.name ?? "منصة ميزان لإستدامة خدمات المياه"}
          </div>
          <div className="hidden md:flex items-center gap-3 text-xs">
            <span className="font-bold">{tenant?.name ?? "غير محدد"}</span>
