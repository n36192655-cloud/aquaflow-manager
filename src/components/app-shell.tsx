...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen flex w-full bg-background text-foreground">
    <aside className="hidden md:flex w-64 shrink-0 flex-col bg-sidebar text-sidebar-foreground border-l border-sidebar-border">
      <div className="px-5 py-6 border-b border-sidebar-border"><div className="flex items-center gap-2"><div className="relative w-9 h-9 rounded-xl grid place-items-center bg-water"><Droplets className="w-5 h-5 text-white" /></div><div><div className="text-lg font-bold tracking-tight">ميزان</div><div className="text-[11px] text-sidebar-foreground/70">منصة ميزان لإستدامة خدمات المياه</div></div></div></div>
    ...
    <div className="flex-1 flex flex-col min-w-0">
      <header className="bg-card border-b px-4 py-2 flex items-center justify-between gap-2"><div className="md:hidden font-bold">منصة ميزان لإستدامة خدمات المياه</div><div className="hidden md:block text-xs text-muted-foreground">{ROLE_LABEL[user.role]} — {user.name}</div><NetworkStatus /></header>
