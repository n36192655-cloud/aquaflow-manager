...
const money = (v: number) =>
  new Intl.NumberFormat("ar-YE", { maximumFractionDigits: 0 }).format(Number(v) || 0);
...
export function CentralDashboard() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      setError(null);
      const { data, error } = await supabase.rpc("central_dashboard_project_metrics", {
        p_days: 30,
      });
  ...
  const efficiency =
    summary.production > 0 ? (summary.consumption / summary.production) * 100 : null;
  ...
  return (
    <div dir="rtl" className="space-y-6 pb-8">
      <header className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            مؤشرات موحّدة لآخر 30 يوماً لجميع المشاريع المرتبطة بالمستأجر المركزي.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        />
        <SummaryCard
          title="كفاءة المياه المجمعة"
          value={pct(efficiency)}
          icon={<CheckCircle2 className="h-5 w-5" />}
        />
        <SummaryCard
          title="نسبة التحصيل المجمعة"
          value={pct(collection)}
        ...
        <CardContent className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-sm">
            <thead>
              <tr className="border-b text-right text-muted-foreground">
                ...
                <th className="p-3">الاستهلاك م³</th>
                <th className="p-3">الإنتاج م³</th>
                <th className="p-3">الكفاءة</th>
                <th className="p-3">التحصيل</th>
                <th className="p-3">قراءات معلقة</th>
              </tr>
            </thead>
            <tbody>
              {projects.map((p) => (
                <tr key={p.tenant_id} className="border-b last:border-0">
                  <td className="p-3 font-medium">{p.project_name}</td>
                  <td className="p-3">
                    <Badge variant={p.subscription_status === "active" ? "default" : "destructive"}>
                  ...
                  <td className="p-3">
                    {p.production_input_m3 == null
                      ? "غير متاح"
                      : Number(p.production_input_m3).toFixed(2)}
                  </td>
                  <td className="p-3">{pct(p.water_efficiency_pct)}</td>
                  <td className="p-3">
                    {pct(p.collection_rate_pct)}{" "}
                    <span className="text-xs text-muted-foreground">
                      ({money(p.collected_amount)})
                    </span>
                  </td>
          ...
          {!projects.length && !loading && (
            <div className="py-8 text-center text-sm text-muted-foreground">
              لا توجد مشاريع تابعة مرتبطة بالمستأجر المركزي.
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardContent className="p-5">
            <div className="text-xs text-muted-foreground">إجمالي الإنتاج المعتمد</div>
            <div className="mt-2 text-2xl font-bold">{summary.production.toFixed(2)} م³</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <div className="text-xs text-muted-foreground">إجمالي الاستهلاك المعتمد</div>
            <div className="mt-2 text-2xl font-bold">{summary.consumption.toFixed(2)} م³</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <div className="text-xs text-muted-foreground">الفواتير / المحصل</div>
            <div className="mt-2 text-2xl font-bold">
              {money(summary.billed)} / {money(summary.collected)}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {summary.pending > 0 && (
        <div className="flex items-center gap-2 rounded-lg border p-3 text-sm">
          <AlertTriangle className="h-4 w-4 shrink-0" /> توجد {summary.pending} قراءة معلقة عبر
          المشاريع وتحتاج مراجعة مديري المشاريع.
        </div>
      )}
      {loading && (
        <p className="text-center text-xs text-muted-foreground">جارٍ تحميل المؤشرات المركزية…</p>
      )}
      <p className="text-xs leading-5 text-muted-foreground">
        هذه اللوحة تقرأ المؤشرات المجمعة من Supabase مباشرة. لا تستخدم بيانات Zustand أو
        localStorage كمصدر حقيقة، ولا تمنح المستأجر المركزي صلاحية تعديل بيانات المشاريع من خلال هذه
        اللوحة.
      </p>
    </div>
  );
