...
export function AiResponseRenderer({ response, onSuggestion }: Props) {
        ...
        <CardContent className="p-4 space-y-3">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <div className="text-[10px] font-mono text-muted-foreground mt-1" dir="ltr">
                {customer.pay_account}
              </div>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2">
            ...
            <StatBox
              label="متأخرات"
              value={fmtYER(totals.arrears)}
              tone={totals.arrears > 0 ? "danger" : undefined}
            />
          </div>
          {series.length > 0 && (
            <div className="h-40">
  ...
  if (response.kind === "loss_analysis") {
    const chart = [
      {
        name: "مياه",
        produced: response.water.produced,
    ...
    return (
      <div className="grid md:grid-cols-2 gap-3">
        <Card className="border-emerald-500/30 bg-emerald-500/5">
          <CardContent className="p-4 space-y-2">
            <div className="flex items-center justify-between">
              <div className="text-sm font-bold flex items-center gap-1.5 text-emerald-700">
                <CheckCircle2 className="w-4 h-4" /> مدفوعة ({response.paid.length})
              </div>
              <Button size="sm" variant="ghost" onClick={() => exportCsv(response.paid, "paid")}>
                <Download className="w-3 h-3 ms-1" /> CSV
              </Button>
            </div>
            <ul className="text-xs space-y-1 max-h-56 overflow-auto">
              {response.paid.map((p) => (
                <li key={p.id} className="flex justify-between border-b pb-1">
                  <span className="font-mono text-[10px]">{p.serial}</span>
                  <span className="flex-1 mx-2 truncate">{p.name}</span>
                  <span className="font-semibold">{fmtYER(p.total)}</span>
                </li>
              ))}
              {response.paid.length === 0 && (
                <li className="text-muted-foreground text-center py-4">لا توجد</li>
              )}
            </ul>
          </CardContent>
        </Card>
        <Card className="border-destructive/30 bg-destructive/5">
          <CardContent className="p-4 space-y-2">
            <div className="flex items-center justify-between">
              <div className="text-sm font-bold flex items-center gap-1.5 text-destructive">
                <XCircle className="w-4 h-4" /> غير مدفوعة ({response.unpaid.length})
              </div>
              <Button
                size="sm"
                variant="ghost"
                onClick={() => exportCsv(response.unpaid, "unpaid")}
              >
                <Download className="w-3 h-3 ms-1" /> CSV
              </Button>
            </div>
            <ul className="text-xs space-y-1 max-h-56 overflow-auto">
              {response.unpaid.map((p) => (
                <li key={p.id} className="flex justify-between border-b pb-1">
                  <span className="font-mono text-[10px]">{p.serial}</span>
                  <span className="flex-1 mx-2 truncate">{p.name}</span>
                  <span className="font-semibold">{fmtYER(p.balance)}</span>
                </li>
              ))}
              {response.unpaid.length === 0 && (
                <li className="text-muted-foreground text-center py-4">لا توجد</li>
              )}
            </ul>
          </CardContent>
  ...
  if (response.kind === "revenue_report") {
    return (
      <Card>
        <CardContent className="p-4 space-y-3">
          <div className="text-xs text-muted-foreground">
            {response.range.label}: {response.range.from} → {response.range.to}
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <StatBox
              label="نقدي"
              value={fmtYER(response.totals.cash)}
              icon={<Wallet className="w-4 h-4 text-water" />}
            />
            <StatBox
              label="الكريمي"
              value={fmtYER(response.totals.bank)}
              icon={<Smartphone className="w-4 h-4 text-primary" />}
            />
            <StatBox
              label="الإجمالي"
              value={fmtYER(response.totals.total)}
              tone="ok"
              icon={<CircleDollarSign className="w-4 h-4 text-emerald-600" />}
            />
            <StatBox
              label="متوسط الدفعة"
              value={fmtYER(response.totals.avg)}
          ...
          {response.series.length > 0 ? (
            ...
            <p className="text-xs text-muted-foreground text-center py-6">
              لا توجد دفعات معتمدة في هذه الفترة
            </p>
          )}
...
function StatBox({
  ...
  return (
    <div className={`rounded-lg border p-2.5 ${cls}`}>
      ...
      <div className={`text-sm font-bold mt-1 ${tone === "danger" ? "text-destructive" : ""}`}>
        {value}
      </div>
    </div>
  );
